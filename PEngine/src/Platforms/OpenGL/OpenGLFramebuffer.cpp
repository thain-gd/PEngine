...
namespace PEngine
{
    ...
    void OpenGLFramebuffer::Invalidate()
    {
		...
		if (m_ColorAttachmentSpecifications.size())
		{
			...
			for (size_t i = 0; i < m_ColorAttachments.size(); i++)
			{
				...
				{
					case FramebufferTextureFormat::RBGA8:
		...
		{
			PE_CORE_ASSERT(m_ColorAttachments.size() <= 4); // only support at most 4 color attachments
    ...
    void OpenGLFramebuffer::Resize(uint32_t width, uint32_t height)
    {
        ...
        {
            PE_CORE_WARN("Attempted to resize framebuffer to {0}, {1}", width, height);
            return;
        }
